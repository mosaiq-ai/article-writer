...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `📄 Processing uploaded file: ${file.name}`,
    ...
    if (sessionId) {
      const added = sessionStore.addDocumentToSession(sessionId, processedDocument.id)
      if (!added) {
        console.warn(`⚠️ Could not add document to session ${sessionId}`)
      } else {
        console.log(`✅ Added document ${processedDocument.id} to session ${sessionId}`)
      }
    }
...
async function processFile(file: File) {
  ...
  try {
    ...
    processedFileType = fileType.includes("pdf")
      ? "pdf"
      : fileType.includes("word")
        ? "word"
        : "text"
