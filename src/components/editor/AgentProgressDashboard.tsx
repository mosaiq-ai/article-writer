  ...
  useEffect(() => {
    ...
    eventSource.onmessage = (event) => {
      ...
      if (state.status === 'completed' && state.results.length > 0) {
        ...
        const finalDocument = typeof finalResult.output === 'object' && finalResult.output 
    ...
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-2">
              <Badge variant={
                flowState.status === 'completed' ? 'default' :
                flowState.status === 'failed' ? 'destructive' :
                'secondary'
              }>
                {flowState.status}
              </Badge>
            ...
            {flowState.endTime && (
              <span className="text-muted-foreground">
                Completed in {Math.round(
                  (new Date(flowState.endTime).getTime() - new Date(flowState.startTime!).getTime()) / 1000
                )}s
              </span>
            )}
          </div>
                        ...
                        {result && (
                          <div className="text-sm text-muted-foreground mt-1">
                            ...
                            {result.metadata.documentsAccessed && (
                              <span> • {result.metadata.documentsAccessed.length} docs accessed</span>
                            )}
                          </div>
                        )}
                        {status === 'error' && result && (
      ...
      {flowState.status === 'completed' && (
        <Card>
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Your document has been generated successfully using the full content of your source documents.
              You can now view the results or download the document.
            </p>
            <div className="flex gap-2">
              <Button 
                className="flex-1"
