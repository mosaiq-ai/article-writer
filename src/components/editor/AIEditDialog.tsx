...
export function AIEditDialog({
  ...
  const generateAIEdit = async () => {
    ...
    setIsLoading(true)
    try {
      const prompt = buildPrompt(action, instruction, selection)
  ...
  const buildPrompt = (action: string, customInstruction: string, selection: any): string => {
...
`
    ...
    const actionPrompts: Record<string, string> = {
      improve: 'Improve the writing quality, clarity, and flow while maintaining the original meaning.',
      fix: 'Fix all grammar, spelling, and punctuation errors.',
      simplify: 'Simplify the language to make it easier to understand.',
      expand: 'Expand this text with more detail and explanation while keeping the same key points.',
      ...
      <DialogContent className="max-w-2xl">
        ...
        <DialogFooter>
          ...
          {action === 'custom' && !result ? (
            <Button onClick={generateAIEdit} disabled={!instruction || isLoading}>
