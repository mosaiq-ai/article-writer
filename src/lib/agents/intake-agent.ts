...
1. List all available documents using listDocuments tool
2. Use getMultipleDocuments tool with these exact document IDs: [${context.documentIds.map((id) => `"${id}"`).join(", ")}]
   IMPORTANT: Use the exact IDs provided above, NOT document titles!
...
Provide a comprehensive analysis that will inform the document creation process.`
      ...
      const result = await aiService.generateText(prompt, {
        ...
        ${context.sessionId ? `\nWhen calling document tools, include sessionId: "${context.sessionId}" in your tool calls.` : ""}
